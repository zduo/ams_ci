...
<form class="form-horizontal" method="post" action="<?=current_url()?>" autocomplete="off">
    <div class="control-group">
        ...
        <div class="controls">
            <input type="text" name="key_str" id="key_str" value="<?=set_value('key_str', $key_info['key_str'])?>" />
        </div>
    </div>
    <div class="control-group">
        <label class="control-label" for="key_map_str">映射词</label>
        <div class="controls">
            <input type="text" name="key_map_str" id="key_map_str" value="<?=set_value('key_map_str', $key_info['key_map_str'])?>" />
        </div>
    </div>
    <div class="control-group">
        <div class="controls">
            <input type="hidden" name="update_key" value="1" />
            <a class="btn btn-link" href="<?=base_url('admin/search_key')?>">取消</a>
            <button type="reset" class="btn">重置</button>
            <button type="submit" class="btn btn-success">修改映射</button>
        </div>
    </div>
</form>
